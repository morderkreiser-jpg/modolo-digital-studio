...
export default function HeroShowcase({ items }: { items: ShowcaseItem[] }) {
  ...
  return (
    <div className="hsw-wrap mds-in" style={{ animationDelay: "0.4s" }}>
      <style>{CSS}</style>
      <a
        className="hsw-card"
        href={active.href}
...
const CSS = `
.hsw-wrap { width: 100%; max-width: 620px; }
.hsw-card { display: block; border-radius: 12px; overflow: hidden; text-decoration: none;
  ...
  transition: transform 0.4s cubic-bezier(0.16,1,0.3,1), box-shadow 0.4s; will-change: transform; }
.hsw-card:hover { transform: translateY(-4px); box-shadow: 0 4px 8px rgba(126,93,36,0.10), 0 34px 60px -20px rgba(126,93,36,0.5); }
